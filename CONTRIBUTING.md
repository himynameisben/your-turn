...
| `--render <dir>` | Render the main window and settings page to PNGs in every palette |

Add `--demo` to `--render` for invented sessions instead of your real `~/.claude` —
required for anything you attach to a PR. To check the Chinese build, append
`-AppleLanguages '(zh-Hant)'` to the same command.

**Data-layer changes: verify with `--dump`. Layout changes: verify with `--render`.**
...
- Include the relevant `--dump` / `--render` evidence in the PR description.
